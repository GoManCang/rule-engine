 ...
 * @author zhengbaiyun
 */
public class StatelessModelRuleEngine extends RuleEngine {

    private static final Logger logger = LoggerFactory.getLogger(StatelessModelRuleEngine.class);
    private StatelessKnowledgeSession statelessKSession;
    /**
     * Cache, Key为: packageName
     */
    private Map<String, ModelRule> modelRulesInKBase = Maps.newHashMap();
    ...
    public void updateRules() {
        ...
        Map<String, ModelRule> newModelRulesInKBase = Maps.newHashMap();

        // 删除&更新Route规则
        for (ModelRule rule : Configs.getModelRules()) {
            String packageName = rule.getRuleNo();
            ModelRule ruleInKBase = this.modelRulesInKBase.get(packageName);
            if (!rule.isEnabled()) {
                if (ruleInKBase != null) {
                    try {
                ...
                newModelRulesInKBase.put(packageName, rule);
            }
        }
        logger.warn("exec updateRules() end.");

        // 最后更新缓存
        this.modelRulesInKBase = newModelRulesInKBase;
    }
    ...
    public boolean isRuleInKBase(String packageName) {
        return modelRulesInKBase.containsKey(packageName);
    }
