 ...
 * @author zhengbaiyun
 */
public class StatelessWhitelistRuleEngine extends RuleEngine {

    private static final Logger logger = LoggerFactory.getLogger(StatelessWhitelistRuleEngine.class);
    private StatelessKnowledgeSession statelessKSession;
    /**
     * Cache, Key为: packageName
     */
    Map<String, WhitelistRule> whitelistRulesInKBase = Maps.newHashMap();
    ...
    public void updateRules() {
        ...
        Map<String, WhitelistRule> newWhitelistRulesInKBase = Maps.newHashMap();

        // 删除&更新Route规则
        for (WhitelistRule rule : Configs.getWhitelistRules()) {
            String packageName = rule.getRuleNo();
            WhitelistRule ruleInKBase = this.whitelistRulesInKBase.get(packageName);
            if (!rule.isEnabled()) {
                if (ruleInKBase != null) {
                    try {
                ...
                newWhitelistRulesInKBase.put(packageName, rule);
            }
        }
        logger.warn("exec updateRules() end.");

        // 最后更新缓存
        this.whitelistRulesInKBase = newWhitelistRulesInKBase;
    }
    ...
    public boolean isRuleInKBase(String packageName) {
        return whitelistRulesInKBase.containsKey(packageName);
    }
